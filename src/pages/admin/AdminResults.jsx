...
function AdminResults() {
    ...
    const openEdit = (r) => { setSelected(r); setForm({ title: r.title, year: r.year || '', age: r.age || '' }); setIsDirty(false); setModal('edit'); };
                        ...
                        <table className="admin-table">
                            ...
                            {list.map(r => (
                                <tr key={r.id}>
                                    ...
                                    <td>{r.year || '—'}</td>
                                    <td><span style={{background:'#eef4fb',color:'#4a90d9',padding:'3px 10px',borderRadius:'20px',fontSize:'12px',fontWeight:700}}>{getAgeLabel(r.age)}</span></td>
                                    <td><span style={{color:'#6b7a8d',fontSize:'13px'}}>{r.categoryResults?.length || 0} categorii</span></td>
                                    <td>
                                        <div className="admin-actions">
                                            <button className="admin-edit-btn" onClick={() => navigate(`/admin/results/${r.id}/edit`)} style={{background:'#e8f4e8',color:'#28a745'}}>🏅 Categorii</button>
